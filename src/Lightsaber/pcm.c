...
ISR(TIMER1_COMPA_vect) {
  if (sample < firstSample) {
    // Ramp up from zero to reduce click at beginning of playback
    OCR2A = sample;
  }
  else if (sample - firstSample >= sounddata_length) {
    if (sample - firstSample == sounddata_length + lastSample) {
      stopPlayback();
    }
    else {
      // Ramp down to zero to reduce the click at the end of playback.
      OCR2A = sounddata_length + lastSample - (sample - firstSample);
    }
  }
  else {
    OCR2A = pgm_read_byte(&sounddata_data[sample - firstSample]);
  }
...
void startPlayback(unsigned char const *data, int length, int rate)
{
  ...
  OCR1A = F_CPU / rate;    // 16e6 / 8000 = 2000
  
  // Enable interrupt when TCNT1 == OCR1A (p.136)
  TIMSK1 |= _BV(OCIE1A);

  firstSample = pgm_read_byte(&sounddata_data[0]);
